...
class ApiService {
  ...
  static Future<void> init() async {
    ...
    _imagesUrl = dotenv.get('CRADOS_IMAGES_URL', fallback: 'http://localhost/Node-API-Crados/BACKEND/FILES/');
  }
  ...
  static Future<List<Crados>> fetchAllCrados() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Échec du chargement des Crados: ${response.statusCode}');
      }
    } catch (e) {
  ...
  static Future<Crados> fetchRandomCrados() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Échec du chargement du Crados aléatoire: ${response.statusCode}');
      }
    } catch (e) {
  ...
  static Future<Crados> fetchCradosById(int id) async {
    try {
      final response = await http.get(Uri.parse('$_apiUrl/api/v1/crados/id/$id'));
      
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        return Crados.fromJson(data);
      } else {
        throw Exception('Échec du chargement du Crados avec ID $id: ${response.statusCode}');
      }
    } catch (e) {
      throw Exception('Erreur lors de la récupération du Crados avec ID $id: $e');
    }
  }
  ...
  static Future<Crados> fetchCradosByName(String name) async {
    try {
      final response = await http.get(Uri.parse('$_apiUrl/api/v1/crados/name/$name'));
      
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        return Crados.fromJson(data);
      } else {
        throw Exception('Échec du chargement du Crados avec nom $name: ${response.statusCode}');
      }
    } catch (e) {
      throw Exception('Erreur lors de la récupération du Crados avec nom $name: $e');
    }
  }
